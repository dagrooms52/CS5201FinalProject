...
// Course: CS5201
// Date: 4/20/2015
// File Name: symmatrix.h
// Purpose: To define a class for a Symmetric Matrix, which derives from the 
...
// Function: SymMatrix(const int size)
// Pre: int size > 0
...
// Function: SymMatrix(SymMatrix copy)
// Pre: None
...
// Function: operator=(SymMatrix rhs)
// Pre: None
...
template <class T>
class SymMatrix : public AbstractMatrix<T>
{
  private:
  ...
  public:
    ...
    // Default Constructor
    SymMatrix<T>(): m_size(0), m_data(NULL) {};

    // Size Constructor
    SymMatrix<T>(const int size);
    ...
    // Copy constructor
    SymMatrix<T>(const SymMatrix<T> & cpy);
    ...
    // Input operator
    template <class U>
    friend std::istream& operator>> (std::istream& in, SymMatrix<U>& mx);

    // LMatrix addition - more needed
    SymMatrix<T> operator+ (const SymMatrix<T>& mx) const;

    // LMatrix subtraction - more needed
    SymMatrix<T> operator- (const SymMatrix<T>& mx) const;

    // LMatrix multiplication - more needed
    SymMatrix<T> operator* (const SymMatrix<T>& rhs) const;

    // Scalar multiplication - only 1
    SymMatrix<T> operator* (const T scalar) const;
