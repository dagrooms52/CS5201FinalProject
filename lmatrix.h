...
// Daate: 3/30/2015
// File Name: lmatrix.h
// Purpose: To define a class for a Lower Triangle Matrix, which derives from the 
...
// Function: LMatrix(const int size)
// Pre: int size > 0
...
template <class T>
class LMatrix : public AbstractMatrix<T>
{
  private:
  ...
  public:
    ...
    // Input operator
    template <class U>
    friend std::istream& operator>> (std::istream& in, LMatrix<U>& mx);

    // LMatrix addition - more needed
    LMatrix<T> operator+ (const LMatrix<T>& mx) const;

    // LMatrix subtraction - more needed
    LMatrix<T> operator- (const LMatrix<T>& mx) const;

    // LMatrix multiplication - more needed
    LMatrix<T> operator* (const LMatrix<T>& rhs) const;

    // Scalar multiplication - only 1
    LMatrix<T> operator* (const T scalar) const;
